...
if __name__ == "__main__":
    # input/output dirs
    input_dir = osp.join(osp.dirname(__file__), "smplx_params")
    ...
    for f in pbar:
        subset_name = f.split("+__+")[0]
        subject = f.split("+__+")[1]
        action = f.split("+__+")[2][:-4]

        curr_output_dir = osp.join(output_dir, subset_name, f[:-4])

        os.makedirs(curr_output_dir, exist_ok=True)

        raw = np.load(osp.join(input_dir, f), allow_pickle=True).item()
        ...
        save_dir = osp.join(curr_output_dir, "smplx_humanoid_fingers")
